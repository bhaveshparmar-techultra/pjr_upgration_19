...
class SaleOrder(models.Model):
    ...
    number_to_words_sub_total = fields.Char(string="Amount in Words (SubTotal) :",
                                      compute='_compute_number_to_words',
                                      help="To showing total amount in words")
    ...
    def _compute_number_to_words(self):
        ...
        for rec in self:
            rec.number_to_words = rec.currency_id.amount_to_text(
                rec.amount_total)
            # Use amount_untaxed for subtotal (before taxes)
            rec.number_to_words_sub_total = rec.currency_id.amount_to_text(
                rec.amount_untaxed)
