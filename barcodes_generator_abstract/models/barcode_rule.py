...
class BarcodeRule(models.Model):
    ...
    def _clear_cache(self, vals):
        ...
        cache_fields = ("generate_model", "generate_automate")
        if isinstance(vals, list):
            # For create with vals_list
            if any(any(k in v for k in cache_fields) for v in vals):
                self.env.registry.clear_cache()
        else:
            # For write with vals dict
            if any(k in vals for k in cache_fields):
                self.env.registry.clear_cache()
